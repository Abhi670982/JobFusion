...
import {
  ...
  Lightbulb, Target, Code2, XCircle, Info, ArrowLeft
} from 'lucide-react';
...
export default function ResumePage() {
  ...
  const processFile = async (file: File) => {
    if (!user) {
      setError('You must be logged in to upload a resume. Please refresh or check database connection.');
      return;
    }
    setError(null);
    ...
    try {
      ...
      if (result.success) {
        setSuccessMessage(`Resume uploaded! ${result.data.skillsExtracted} skills extracted.`);
        setProfile(await fetchProfile(user._id));
      } else {
  ...
  const handleManualParse = async () => {
    ...
    try {
      ...
      if (result.success) {
        setSuccessMessage(`Re-parsed! ${result.data.skillsExtractedCount} skills extracted.`);
        setProfile(await fetchProfile(user._id));
  ...
  const handleDeleteResume = async () => {
    ...
    if (!confirm('Are you sure you want to remove your resume? This will also clear all skills and career insights.')) return;
    try {
      const updated = await updateProfile(user._id, {
        ...
        resumeSummary: '',
        suggestedRoles: [],
        resumeInsights: { found: [], missing: [], tips: [] }
      });
      if (updated) {
        setProfile(updated);
        setSuccessMessage('Resume removed and skills cleared.');
      }
    } catch (err: any) {
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              ...
              {uploading ? (
                ...
                <>
                  ...
                  <Button size="sm" type="button" onClick={(e) => { e.stopPropagation(); fileInputRef.current?.click(); }} className="rounded-xl gradient-brand text-white border-0 hover:opacity-90 text-xs">
          ...
          <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  ...
                  <div className="p-2.5 rounded-xl bg-muted/40 text-center">
                    <div className="text-lg font-extrabold text-foreground">
                      {profile.resumeUpdatedAt ? new Date(profile.resumeUpdatedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }) : '—'}
                    </div>
            ...
            {insights && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="card-premium p-5 space-y-4">
                ...
                {insights.found && insights.found.length > 0 && (
                  <div className="space-y-2">
                ...
                {insights.missing && insights.missing.length > 0 && (
                  <div className="space-y-2">
